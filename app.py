...
def main():
    ...
    if st.session_state.user_name:
        ...
        with col1:
            # Κλητική πτώση απλή (αν τελειώνει σε "ς", αφαιρείται και προστίθεται "α")
            vocative_name = (
                st.session_state.user_name[:-1] + 'α' if st.session_state.user_name.endswith('ς') else st.session_state.user_name
            )
    ...
    # Προσθήκη επιπλέον εργασιών για Φεβρουάριο
    if "Φεβρουάριος" in tasks_by_month:
        extra_feb_tasks = [
        ...
        tasks_by_month["Φεβρουάριος"].extend(extra_feb_tasks)
